...
func unmarshalPacket(data []byte, p *Packet, readBody bool) error {
	...
	if readBody {
		...
		if err := binary.Read(reader, binary.NativeEndian, p.body); err != nil {
			return err
		}
	}
	return nil
...
func unmarshalTLVMsg(data []byte, t *TLVMsg, readBody bool) error {
	...
	if readBody {
		...
		if err := binary.Read(reader, binary.NativeEndian, t.body); err != nil {
			return err
		}
	}
	return nil
...
func unmarshalSeqedMsg(data []byte, s *SeqedMsg, readBody bool) error {
	...
	if readBody {
		...
		if err := binary.Read(reader, binary.NativeEndian, s.body); err != nil {
			return err
		}
	}
	return nil
...
func UmarshalBodyOnly(bodyData []byte, bodyLen int, p ziface.IPacket) error {
