...
type WokerPool struct {
	size   int           // 协程池大小
	mq     ziface.IQueue     // 消息队列（引用）
	mapper ziface.IApiMapper    // API映射（引用）
	stopCh chan struct{} // 停止信号通道
	wg     sync.WaitGroup
}

func NewWokerPool(size int, mq ziface.IQueue, mapper ziface.IApiMapper) *WokerPool {
	return &WokerPool{
		size:   size,
		mq:     mq,
		mapper: mapper,
...
func (w *WokerPool) Post(request ziface.IRequest) {
	w.mq.Push(request)
}

// processRequest 执行具体的业务逻辑
func (w *WokerPool) processRequest(request ziface.IRequest) error {
	return w.mapper.ExecJob(request.Msg().Tag(), request)
}
